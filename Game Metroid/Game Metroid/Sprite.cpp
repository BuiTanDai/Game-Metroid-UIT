...
Sprite::Sprite(GTexture* texture, int start, int end, int timeAnimation)
: _texture(texture)
{
	_start = start;
	_end = end;
...
Sprite::Sprite(GTexture* texture, int timeAnimation)
: _texture(texture)
{
	_start = 0;
...
void Sprite::DrawRaw(int X, int Y)
{
	...
	srect.left = (_index % _texture->Cols - 1)*(_texture->FrameWidth);// + 1;
	srect.top = (_index / _texture->Cols)*(_texture->FrameHeight);// + 1;
	srect.right = srect.left + _texture->FrameWidth;
	srect.bottom = srect.top + _texture->FrameHeight;// + 1;
